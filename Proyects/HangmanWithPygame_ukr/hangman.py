	...
	if modal_context == 'scoreboards':
		...
		if (len(hi_scores.keys()) > 0):
			...
			if (len(hi_scores.keys()) == 1):
				# Add sub-title
				blit_text(str(list(hi_scores.keys())[0])+" letter words ("+difficulty(list(hi_scores.keys())[0])+")", (255, 0, 0), (0, 95), window, font2, True)
				# Get scores based on word length
				single_score = hi_scores[list(hi_scores.keys())[0]][0]
...
def previous_scoreboard():
	...
	if scoreboard_to_show_key > 0:
		scoreboard_to_show_key = (scoreboard_to_show_key-1) if (list(hi_scores.keys())[scoreboard_to_show_key-1]) in hi_scores.keys() else (len(hi_scores.keys())-1)
	else:
		scoreboard_to_show_key = len(hi_scores.keys()) -1
	current_scoreboard_wordcount = list(hi_scores.keys())[scoreboard_to_show_key]
	scoreboard_to_show = hi_scores[list(hi_scores.keys())[scoreboard_to_show_key]]

def next_scoreboard():
	global scoreboard_to_show_key, scoreboard_to_show, current_scoreboard_wordcount
	if scoreboard_to_show_key < len(hi_scores.keys()) - 1:
		scoreboard_to_show_key = (scoreboard_to_show_key+1) if (list(hi_scores.keys())[scoreboard_to_show_key+1]) in hi_scores.keys() else 0
	else:
...
try:
	...
	raise  "could not load or play soundfiles"
		...
		# Keydown event (making sure our key id is within character evaluation range)
			...
			if modal == True:
				if (event.key == pygame.K_RETURN) and (modal_context == 'submit_score'):
					...
					scoreboard_to_show_key =next(iter(hi_scores.keys()))
